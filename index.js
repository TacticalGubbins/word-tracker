...
client.on("message", (message) => {
  ...
  con.query('SELECT prefix FROM servers WHERE id = ' + message.guild.id, (err, prefixResponse) => {
    ...
    switch(true) {
      case (message.channel.type === 'dm' && (message.content.toLowerCase().startsWith("n!help") || message.content.toLowerCase().startsWith("help"))):
        let embed = new MessageEmbed()
        .setTitle('Bot Help')
        .setColor(0xBF66E3)
        .setDescription('')
        ...
        return;
        break;
      case (message.channel.type === 'dm' && (message.content.toLowerCase() === 'nigga' || message.content.toLowerCase() === 'nigger')):
        try {
          client.guilds.cache.get('687077613457375438').member('250408653830619137').send("**Message from " + message.author.username + ":** " + message.content + "");
        }
        catch(err) {
        ...
        if(client.users.cache.get(user.toString()) !== undefined) {
          con.query('SELECT words FROM users WHERE id = ' + user + ' AND server_id = ' + message.guild.id, (err, rows) => {
            ...
            if(rows[0] === undefined || rows[0].words === 0){
              checkEmbed.setDescription('That user hasn\'t sent any countable words!')
            }
            else {
              checkEmbed.setDescription(client.users.cache.get(user).tag + " has sent **__" + rows[0].words + "__** countable words!");
            }
